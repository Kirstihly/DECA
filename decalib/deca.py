...
class DECA(nn.Module):
    ...
    def decode(
        ...
        if rendering:
            ...
            ops = self.render(
                verts, trans_verts, albedo, h=h, w=w, background=background
            )
            ## output
        ...
        if return_vis:
            ...
            detail_normal_images = (
                F.grid_sample(uv_detail_normals, grid, align_corners=False)
                * alpha_images
            )
    ...
    def save_obj(self, filename, opdict):
        ...
        displacement_map = (
            opdict["displacement_map"][i].cpu().detach().numpy().squeeze()
        )
